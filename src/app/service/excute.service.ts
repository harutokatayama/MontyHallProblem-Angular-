...
export class ExcuteService {
  ...
  public confirmChangeDoorFlag = false;
  // 扉を変更すると選択
  ...
  public disabledDoors(selectedDoor, doors) {
    ...
    for (let n = 0; n < doors.length; n++) {
      if (doors[selectedDoor] === 'Win') {
        ...
        const el = document.getElementById(`${doorNumber}`);
        const el2 = document.getElementById(`${selectedDoor}`);
  ...
  public removeDisabled() {
    ...
    if (!this.confirmChange) {
      const el = document.querySelectorAll('.door');
      el.forEach(e => {
